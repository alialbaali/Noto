...
class SignSharedViewModel(private val userUseCases: UserUseCases) : ViewModel() {
    ...
    val getUserToken = liveData {
        userUseCases.getUserToken().apply {
            emit(this)
        }
    }
    ...
    fun createUser() {
        viewModelScope.launch {
            ...
            when {
                ...
                user?.userDisplayName?.isEmpty() == true ->
                ...
                user?.userPassword?.matches(PASSWORD_REGEX) == false ->
                    ...
                    userUseCases.createUser(user!!).exceptionOrNull()?.let { exception ->
                        _error.postValue(exception.message)
                    }.also {
                        if (it == null) _navigate.postValue(true)
                    }
            ...
            when {
                ...
                user?.userPassword?.isEmpty() == true ->
                    ...
                    userUseCases.loginUser(user!!).exceptionOrNull()?.let {
                        ...
                        if (it == null) {
                            userUseCases.fetchData().onSuccess {
