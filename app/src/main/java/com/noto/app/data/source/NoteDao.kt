...
interface NoteDao : LocalNoteDataSource {
    ...
    @Query("""
        SELECT * FROM 
        notes AS notes 
        JOIN libraries AS libraries 
        ON libraries.id == notes.library_id 
        WHERE notes.is_archived = 0 AND libraries.is_vaulted == 0 AND libraries.is_archived = 0 ORDER BY id DESC
        """)
