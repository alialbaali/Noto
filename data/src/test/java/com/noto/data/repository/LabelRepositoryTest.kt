...
private val labelRepositoryModule = module {
    ...
    single { LabelRepositoryImpl(get<FakeLabelDao>()) }
...
class LabelRepositoryTest : KoinTest, StringSpec() {
    ...
    private val labelRepository by inject<LabelRepositoryImpl>()
    ...
    private val labels by lazy { labelRepository.getLabels().run { runBlocking { single() } }.getOrThrow() }
    ...
    init {
        ...
        "update label" {
            ...
            labels.find { it.labelId == updatedLabel.labelId } shouldNotBe null
            labels.find { it.labelId == updatedLabel.labelId }!!.labelTitle shouldBe updatedLabel.labelTitle
            labels.find { it.labelId == updatedLabel.labelId }!!.labelColor shouldBe updatedLabel.labelColor

        }

        "get label by id"{

            val result = labelRepository.getLabel(label.labelId).single().getOrThrow()

            result shouldBe updatedLabel
            result shouldNotBe label
            result.labelId shouldBeExactly label.labelId
        ...
        "remove label"{
            ...
            labelRepository.deleteLabel(label.labelId)
